 ...
 */
const REQUEST_RESPONSE_TIME = 2000
 ...
 */
export const fetchProviders = (region, lang) => {
  return new Promise(
    (resolve) => setTimeout(
      () => 
        {
          const matchingProviders = fullDataset.filter(({languages, location}) =>  languages.includes(lang) && location.includes(region))
          resolve(matchingProviders ? matchingProviders.map(makePartialRepresentation) : [])
        },
      REQUEST_RESPONSE_TIME
    )
  )
